...
from nn_generator.model_generator.simple_nn import SimpleNN, save_model
...
def extract_features(timestamp, features=1, raw_feature=True):
    """ Unpacks the features of a given timestamp

    @timestamp: datetime.timestamp

    returns: 1x4 vector ["day of the week",
    "time of the day", "season", "study phase"]
    """
    ...
    if features == 2:
        if not raw_feature:
            return numpy.array([day_of_the_week, time_of_the_day + 1])
        return numpy.array([day_of_the_week, time_of_the_day])
    elif features == 3:
        if not raw_feature:
            return numpy.array([day_of_the_week, time_of_the_day, season])
        return numpy.array([day_of_the_week, time_of_the_day, season - 1])
    elif features == 4:
